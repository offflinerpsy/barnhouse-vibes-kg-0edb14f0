 ...
 * - Социальные сети (WhatsApp, Telegram, Instagram)
 * - Навигация по секциям
 * - Контактная информация
 * - Копирайт
 * - Декоративный skyline с силуэтами домов и мечетью
 * 
 * =============================================================================
 */
...
function TextShimmer({ children, className = "" }: { children: React.ReactNode; className?: string }) {
  return (
    <motion.span
      className={`relative inline-block bg-gradient-to-r from-white via-primary to-white bg-[length:300%_100%] bg-clip-text text-transparent ${className}`}
      ...
      <div className="container mx-auto px-4">
        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-8 lg:gap-6">
          ...
          <div className="lg:col-span-2">
            ...
            >
              <TextShimmer>ERA Concept Home</TextShimmer>
            </motion.h3>
            
            <motion.p 
              className="text-white/70 leading-relaxed max-w-md mb-6 text-sm"
              initial={{ opacity: 0, y: 10 }}
              whileInView={{ opacity: 1, y: 0 }}
              viewport={{ once: true }}
              transition={{ duration: 0.5, delay: 0.3 }}
            >
              Ваш надёжный партнёр в создании современного 
              модульного жилья. Мы делаем мечту о собственном доме доступной.
            </motion.p>
            
            {/* Статистика */}
            <div className="flex gap-8 mb-6">
              <AnimatedStat value="50+" label="проектов" />
              <AnimatedStat value="10+" label="лет опыта" />
              <AnimatedStat value="5" label="лет гарантии" />
            </div>
