      ...
      <div className="container mx-auto px-4 relative z-10" ref={containerRef}>
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 gap-6 lg:gap-8">
          {advantages.map((item, index) => (
            ...
            >
              <motion.div
                className="relative bg-gradient-to-br from-warm-white/5 to-warm-white/[0.02] backdrop-blur-sm rounded-2xl p-6 md:p-8 border border-warm-white/10 overflow-hidden h-full"
                whileHover={{ 
              ...
              >
                ...
                <FloatingLine delay={index * 0.5 + 1.5} />
                
                {/* Number with animation */}
                <div className="flex items-start justify-between mb-6">
                  <div className="relative">
                    <motion.div
                      className="text-5xl md:text-6xl font-bold text-primary/20 font-display"
                      animate={isInView ? {
                  ...
                  {/* Highlight badge */}
                  <motion.span
                    className="text-xs font-medium text-primary bg-primary/10 px-3 py-1.5 rounded-full border border-primary/20"
                ...
                {/* Content */}
                <motion.h3
                  className="font-display text-xl md:text-2xl font-semibold text-warm-white mb-3"
                  initial={{ opacity: 0 }}
                ...
                <motion.p
                  className="text-warm-gray leading-relaxed text-sm md:text-base"
                  initial={{ opacity: 0 }}
                  whileInView={{ opacity: 1 }}
                  viewport={{ once: true }}
                  transition={{ duration: 0.4, delay: 0.3 + index * 0.1 }}
                ...
                >
                  <motion.div
                    className="absolute bottom-4 right-4 w-8 h-8 border-r-2 border-b-2 border-primary/30 rounded-br-lg"
                    animate={{
                      opacity: [0.3, 0.6, 0.3],
                      scale: [1, 1.1, 1],
                    }}
                ...
                {/* Hover glow effect */}
                <motion.div
                  className="absolute inset-0 bg-gradient-to-br from-primary/0 via-primary/0 to-primary/0 rounded-2xl pointer-events-none"
                  whileHover={{
